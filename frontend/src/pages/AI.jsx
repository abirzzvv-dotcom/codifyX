...
export default function AI() {
  ...
  async function loadHistory() {
    try {
      ...
      const data = await api.get(`/ai/history${qs}`);
  ...
  async function clearHistory() {
    ...
    try {
      const qs = selectedProject ? `?project_id=${selectedProject}` : "";
      await api.delete(`/ai/history${qs}`);
      setMessages([]);
    } catch {}
  }
  ...
  async function sendMessage(e) {
    ...
    try {
      ...
      if (data.actions && data.actions.length > 0) {
        const summary = data.actions.map((a) => `${a.action}: ${a.path}`).join(", ");
        setMessages((prev) => [
          ...prev,
          { role: "assistant", content: `Applied changes: ${summary}`, isAction: true },
        ]);
      }
    } catch (err) {
      setMessages((prev) => [
    ...
    <div className="layout">
      ...
      <div className="main-content">
        <div className="page-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            <h1>AI Assistant</h1>
            <p>Powered by NVIDIA Gemma 3n — debug, edit, and create code</p>
          </div>
        ...
        <div className="chat-wrap">
          <div className="chat-messages">
            ...
            {messages.map((m, i) => (
              <div key={i} className={`chat-msg ${m.role} ${m.isAction ? "action" : ""}`}
                style={m.isAction ? { background: "rgba(99,102,241,0.1)", color: "var(--accent)", fontSize: 12 } : {}}>
          ...
          <form className="chat-input-row" onSubmit={sendMessage}>
            <input
              ...
              placeholder="Ask anything... 'Fix the error in my bot', 'Add a /ping command', 'Explain this code'"
              disabled={loading}
            />
